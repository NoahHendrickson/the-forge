...
- **M4 — Deliver:** sending queues change requests in the dev server; a zero-dependency MCP server + auto-installed `/forge-design` command let your running Claude Code session pull and apply them; the browser verifies computed styles post-HMR and flips drafts to **Implemented**.
...
- **M5 — Dispatch:** a send reaches your open agent session with zero keystrokes where the environment allows (tmux → AppleScript → deeplink ladder, manual `/forge-design` fallback), plus queue hardening (claim timeouts, atomic writes, pruning, shared-secret endpoints).
...
- **M-Next — the same loop on Next.js 15/16:** one package, `forge-mode`, now works on Next (App Router and Pages Router, Turbopack and webpack dev) alongside Vite — same overlay, same panel, same MCP loop, same `/forge-design` and `/forge-watch` commands, no separate install path.
- **M-Publish — on npm, one command to set up:** `forge-mode` ships as a real npm package with a `forge-mode` bin; `npx forge-mode init` detects Vite or Next, adds the dependency, wires the config, mounts `<ForgeDesignMode />`, and prints next steps — no manual config editing required.
- **M-Embed — embedded sessions + unified composer:** the panel now hosts a chat composer, and its **↑** is the only send surface (the old "Send to agent" button is retired). By default a send spawns/reuses an embedded Claude Code session at the project root — your subscription CLI, never API keys — which pulls the edits over MCP, streams its activity back into the panel, and asks in-overlay before running anything beyond plain file edits. Type in the composer to steer the change (or just chat); watch mode and the keystroke ladder remain as fallbacks.
...
It detects Vite or Next, adds `forge-mode` as a dev dependency, wires it into your config, and mounts `<ForgeDesignMode />` if it can — printing `[done]` / `[skip]` / `[manual]` for each step. Anything it can't do automatically (an unrecognized config shape, say) it prints the exact snippet for instead; it never touches git.
...
Either framework: `npx forge-mode init` does the above for you; the manual path is
`npm install -D forge-mode` plus the snippets above.
...
Open the printed URL, hit the **Design** toggle (bottom-right), click an element, and start scrubbing. When you have drafts, press **↑** in the composer at the bottom of the panel — by default an embedded Claude Code session picks the queued edits up over MCP, applies them, and the browser marks your drafts Implemented once computed styles match; type a message alongside to steer the change. Prefer your own terminal? Type `/forge-watch` (stay linked) or `/forge-design` (pull once) in a Claude Code session opened in the same project, or hit **Copy for agent** and paste into any agent.

To use it on your own Vite + React or Next.js project: `npx forge-mode init` — see [SETUP.md](SETUP.md) for the full guide and manual fallback steps.
