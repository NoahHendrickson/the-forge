...
| `chat-styles.ts` | the chat/session-feed CSS fragment (feed rows, error cards, tool steps, approval card, turn-done, thinking dots, bubbles, markdown, diff disclosure, composer) — pure string chunks with the same comment-between-concatenation idiom as overlay.ts; exports `CHAT_CSS` only (shared JS/CSS constants like `TEXTAREA_MAX_PX` live beside overlay.ts's TOKENS) |
