...
- The embedded rung now fires for `agent: 'cursor'` projects too (`EMBEDDED_HARNESSES_SET` in `src/server/endpoints.ts`) — it's the primary path for every harness with an adapter, same as claude-code; the deeplink ladder is its fallback, not its default. `codex` still has no embedded adapter and skips straight to the ladder.
