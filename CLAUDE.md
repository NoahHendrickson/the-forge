...
| `chat-rows.ts` | the session feed's row builders (extracted 2026-07-18 chat-ux polish, same under-1k seam): tool step rows with category icons, the diff disclosure + `diffStats` (+N −M summary), user/assistant bubbles, and the thinking (`.chat-working`), turn-done (`✓ Done · $cost`), error-card, and config rows |
