...
- **Permissions (§3.2 posture, adapter-side):** `session/request_permission` with an **edit-kind** tool call → immediately answer the allow-once option (never touches the registry — the ratified auto-allow). Any other kind → `this.onApproval(<kind>, <command/title, sliced to 120>)`; allow → allow-once option, deny → reject-once option, unset handler → immediate reject-once (fail closed). Option selection is by the Task-1-pinned `kind` field on the options array, NOT hardcoded option ids; never the allow-always option. Answer uses the Task-1-pinned response envelope.
