...
## 8b. Ratified constraints from Noah (2026-07-22)

Reacting to this doc, Noah set three bars any studio design must clear:

1. **Spin-up must stay trivial.** The studio has to be as easy as the plugin: one command, no config, no accounts, no containers. Concretely achievable: `npx forge-mode studio` with zero setup — the hub auto-discovers forge-enabled projects from their `.the-forge/` endpoint files (the discovery code already exists), and any project is addable by path. The bar is proven in-genre: Vibe Kanban was literally `npx vibe-kanban`. The overlay can also grow an "open in studio" affordance so the plugin funnels into it.
2. **No UX or responsiveness regression; nothing laggy.** The architectural rule that guarantees this: **every latency-critical interaction stays inside the iframe, where it already runs today.** Hover outlines, selection, draft inline-style application, ripple, verifier — all in-frame satellite code, unchanged. The only *hot* path that crosses the shell↔iframe boundary is panel scrubbing (a NumberField drag driving a draft), and that's a rAF-coalesced postMessage carrying a tiny delta — loopback postMessage is sub-millisecond, so 60fps scrubbing survives; the "React never re-renders while scrubbing" guarantee is untouched because the satellite still applies styles directly. Everything else crossing the boundary is low-frequency (selection changed, send, chat turns). The hub's HTTP proxy hop is the same pattern as the Next sidecar rewrites proxy, which measured 23ms end-to-end in the embedded-sessions E2E — imperceptible for chat streaming.
3. **iframe performance is first-class, not an afterthought.** What the research supports: an iframed dev server renders natively (no emulation layer — Chrome gives a cross-origin iframe its own renderer process, which *isolates* shell jank from app jank and vice versa); HMR websockets are untouched because we iframe the dev server's real origin. The two real iframe perf risks and their mitigations: (a) **N live previews** — N running apps burn CPU/memory even idle, so only the active project's iframe is mounted; inactive projects show a captured placeholder + status, not a live frame; (b) **canvas zoom over a transformed iframe** — CSS-transformed iframes work (Onlook's multi-frame canvas is built on exactly this) but need pointer-mapping care and can blur at non-1x scales; fallback if it disappoints is keeping zoom *inside* the frame (today's body-transform canvas mode driven over RPC). Both risks get a perf gate in any plan: scrub latency and frame-rate measured in a real browser before a milestone ships, same posture as the existing E2E rules.

## 9. Ratified decisions (Noah, 2026-07-22)

1. **Identity: companion, plugin stays lead.** The plugin remains the product ("design mode for your app"); the studio ships as an optional layer for multi-project work, promotable later if it earns it. Corollary: the in-app overlay stays a first-class standalone mode.
2. **Runtime topology: hub owns sessions (T2).** The hub hosts each project's `SessionManager`; dev servers keep only the design-mode runtime. Sessions survive dev-server restarts — including agent-caused ones (agent edits `vite.config.ts` → Vite restarts → under T1 the agent would have killed its own session mid-turn) — and chat works before/without a preview. Requires a handoff rule so plugin-standalone mode (no hub) keeps today's dev-server-owned sessions.
3. **Project open: auto-start the dev server.** Click a project → hub starts its dev server if not running → live preview in seconds. The hub learns each project's dev command via the same package.json sniffing `forge-mode init` does. With №2, the hub is the lifecycle owner for both dev servers and sessions.
4. **v1 scope: one session per project**, on the live working tree (today's model — the preview shows the tree the agent edits). Parallel sessions with worktree isolation become a later per-session environment option (Claude-desktop-picker style), not the v1 default.

Defaults adopted unless Noah objects (not formally decided): **packaging** — `studio` rides the existing `forge-mode` package as a bin subcommand (one npx entry, zero-dep posture intact, own budget line per §8.4); **C2 sequencing** — the Codex adapter (`codex app-server`) lands alongside O1, since three harnesses is what makes the orchestrator story real.
