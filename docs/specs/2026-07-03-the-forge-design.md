...
- **Open (M2b/M5, from first real user session 2026-07-04):** layout-ripple indicator — when a drafted change reflows non-selected elements (e.g. flex `align-items: stretch` making a sibling grow with the selected card), show a secondary outline/badge on visibly affected elements so designers can distinguish layout ripple from mis-targeting. Also consider surfacing the relevant alignment control (align-self) contextually. Panel gains alignment controls in M2b anyway (Layout section).
